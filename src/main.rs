...
async fn mcp_command(args: McpArgs) -> anyhow::Result<()> {
    ...
    let search_tool = Tool {
        ...
        description: Some("Search indexed content".to_string()),
        input_schema: ToolInputSchema {
            r#type: "object".to_string(),
            properties: Some(
                [(
    ...
    server.register_tool_handler("search", |params: Value| async move {
        ...
        // Create database connection
        let db = match hal::index::Database::new_local_libsql().await {
            Ok(db) => db,
            Err(e) => return Err(MCPError::Internal(format!("Failed to connect to database: {}", e))),
        };
        ...
        // Search the index
        let results = match hal::search::search_index_with_client(&db, &client, query, options).await {
            Ok(results) => results,
            Err(e) => return Err(MCPError::Internal(format!("Search failed: {}", e))),
        };
    ...
    match server.serve(transport).await {
        Ok(_) => Ok(()),
        Err(e) => Err(anyhow!("MCP server error: {}", e)),
    }
}
