    ...
    C: CompletionModel + Clone + Send + Sync + 'static,
{
    ...
    /// Definitions of the tools available to the Pro agent.
    /// These are sent to the model to inform it about available functions.
    /// Using Arc to make cloning the config cheaper if tool defs are large/numerous.
    pub pro_tool_defs: Arc<Vec<ToolDefinition>>,

    /// Maximum number of iterations the Pro agent loop should run.
    /// Helps prevent infinite loops or excessive execution time.
    pub max_pro_iterations: usize,
    ...
    C: CompletionModel + Clone + Send + Sync + 'static,
{
    ...
    pub fn new(
        pro_agent: Arc<Agent<C>>,
        junior_agent: Agent<C>,
        tool_defs: Vec<ToolDefinition>,
        max_junior_iterations: usize,
        pro_tool_defs: Vec<ToolDefinition>,
        max_pro_iterations: usize,
    ) -> Self {
        Self {
            pro_agent,
            ...
            pro_tool_defs: Arc::new(pro_tool_defs),
            max_pro_iterations,
        }
    }
}
