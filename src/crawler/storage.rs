...
fn get_storage_path(url: &str) -> Result<std::path::PathBuf> {
    ...
    let base_path = Path::new(".hal").join("crawler").join(domain);
    Ok(base_path.join(filename))
}

/// Creates necessary directories for storage
async fn ensure_directories(path: &Path) -> io::Result<()> {
...
pub async fn store(entry: &PageEntry) -> Result<()> {
    let storage_path = get_storage_path(&entry.url)?;
    ensure_directories(&storage_path).await?;
...
pub async fn load(url: &str) -> Result<PageEntry> {
    let storage_path = get_storage_path(url)?;
...
mod tests {
    ...
    fn test_get_storage_path() {
        let path = get_storage_path("https://example.com/").unwrap();
